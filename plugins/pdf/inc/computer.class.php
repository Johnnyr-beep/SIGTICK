...
class PluginPdfComputer extends PluginPdfCommon
{
    ...
    {
        PluginPdfCommon::actaSantacruz($pdf, $computer);
    }
    ...
    public static function pdfOperatingSystem(PluginPdfSimplePDF $pdf, Computer $computer)
    {
        ...
        if ($user->getFromDB($computer->fields['users_id'])) {

            $nombre = $user->getFriendlyName();

            $cedula = $user->fields['registration_number'] ?? 'No registrada';

            $cargo = $user->fields['usertitles_id']
                ? Dropdown::getDropdownName(
                    'glpi_usertitles',
                    $user->fields['usertitles_id']
                )
                : 'No definido';
