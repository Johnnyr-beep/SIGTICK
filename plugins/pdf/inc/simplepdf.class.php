...
class PluginPdfTCPDF extends TCPDF
{
    public function Header()
    {
        ...
        $logo_h     = 15; // keep in sync with SetHeaderData height
        $imgw       = 0;
        if (!empty($headerdata['logo']) && $headerdata['logo'] != K_BLANK_IMAGE) {
            ...
            $imgw = $this->getImageRBX() - $this->original_lMargin;
        }
        $headerfont = $this->getHeaderFont();
        $cell_height = $this->getCellHeight($headerfont[2] / $this->k);
        // Vertically center the title relative to the logo height.
        $title_y = $this->getHeaderMargin() + max(0, ($logo_h - $cell_height) / 2);
        $this->SetTextColorArray($this->header_text_color);
        $this->SetFont($headerfont[0], $headerfont[1], $headerfont[2]);
        $this->SetXY($this->original_lMargin + $imgw, $title_y);
...
class PluginPdfSimplePDF
{
    ...
    **/
    public function setHeader($msg)
    {
        ...
        if (
            ...
            $custom_logo = Plugin::getPhpDir('pdf') . '/custom/LOGOFINAL.jpeg';
            if (file_exists($custom_logo)) {
                $this->pdf->SetHeaderData($custom_logo, 15, $msg, '');
            } else {
    ...
    **/
    private function displayInternal($gray, $padd, $defalign, $miny, $msgs)
    {
        $text_color = null;
        if (is_array($gray)) {
            // Allow [r, g, b] or [r, g, b, [tr, tg, tb]] for fill + text colors
            if (isset($gray[3]) && is_array($gray[3])) {
                $text_color = $gray[3];
            }
            $this->pdf->SetFillColor($gray[0], $gray[1], $gray[2]);
        } else {
    ...
    **/
    ...
    {
        // Santacruz green background with white text
        $this->displayInternal([46, 125, 50, [255, 255, 255]], 1.0, self::CENTER, 1, func_get_args());
    ...
    **/
    ...
    {
        $this->displayInternal(255, 0.5, self::LEFT, 1, func_get_args());
    }
